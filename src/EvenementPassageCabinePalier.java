    ...
    */
    ...
    public void traiter(Immeuble immeuble, Echeancier echeancier) {
        ...
        if(cabine.passagersVeulentDescendre() || étage.aDesPassagers())
            echeancier.ajouter(
                    new EvenementOuverturePorteCabine(
                            date + Global.tempsPourOuvrirOuFermerLesPortes
                    )
            );
        else{
            if( (!this.étage.equals(immeuble.étageLePlusBas()) || cabine.intention() == '^')
             && (!this.étage.equals(immeuble.étageLePlusHaut()) || cabine.intention() == 'v'))
                echeancier.ajouter
                    (new EvenementPassageCabinePalier(
                            date + Global.tempsPourBougerLaCabineDUnEtage,
                            immeuble.étage(étage.numéro() + (cabine.intention() == 'v' ? - 1 : 1))
                    )
                );
        }
    }
